...
interface ClubAlumniPageProps {
  params: Promise<{ id: string }>;
}

export default function ClubAlumniPage({ params }: ClubAlumniPageProps) {
    ...
    return (
      <div className="min-h-screen bg-slate-50 px-6 py-10">
        <div className="max-w-3xl mx-auto text-center">
          <p className="text-red-600">Unable to load alumni.</p>
  ...
  const alumni = (club.members || []).filter((member) => member.isAlumni);
      ...
      <div className="max-w-5xl mx-auto px-6 py-10 space-y-6">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs uppercase tracking-wide text-slate-400">Alumni</p>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {alumni.length === 0 && (
              <p className="text-sm text-slate-500">No alumni listed.</p>
            )}
            {alumni.map((member) => (
              <div
                key={member.uuid}
              ...
              >
                ...
                <div>
                  ...
                  {member.batchYear && (
                    <p className="text-[11px] text-slate-400">Batch {member.batchYear}</p>
                  )}
                </div>
              </div>
            ))}
